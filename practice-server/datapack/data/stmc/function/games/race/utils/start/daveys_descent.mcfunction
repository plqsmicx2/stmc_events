...
# start music
execute at @s run playsound music_disc.5 record @s 0 10000 0 1000 1 1

# make sure checkpoint entities are present
execute in stmc:race/daveys_descent run function stmc:games/race/utils/spawn_checkpoints/daveys_descent

# set player to currently playing
scoreboard players set @s race.gameActive 2
...
# teleport player
execute in stmc:race/daveys_descent run tp @s 51 136 -433 0 0