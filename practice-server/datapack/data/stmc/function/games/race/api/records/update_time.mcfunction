...
$tellraw @a [{selector:"@s"},{text:" set a new Race time record! Map: Davey's $(map)",color:gold,bold:true}]

# store @s's time in ticks
$execute store result storage race:data record.$(map).time int 1 run scoreboard players get @s race.laptime.total
...
$data modify storage race:data record.$(map).name set from entity @n[type=item_display,tag=profile] item.components."minecraft:profile".name
