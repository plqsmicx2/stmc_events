...
tellraw @a [{selector:"@s"},{text:" set a new Race lap record!",color:green}]

# store @s's time in ticks
execute store result storage race:data record.lap.time int 1 run scoreboard players get @s race.fastest_lap
...
data modify storage race:data record.lap.name set from entity @n[type=item_display,tag=profile] item.components."minecraft:profile".name
