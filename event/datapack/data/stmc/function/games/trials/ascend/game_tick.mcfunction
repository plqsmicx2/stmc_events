...
execute in stmc:trials/ascend run kill @e[type=item,nbt={Item:{id:"minecraft:fishing_rod"}}]

# and check for return item use
execute as @a if score @s ascend.return matches 1.. run function stmc:games/trials/ascend/spawn_return
...
execute as @a[team=RED_RACCOONS] if score @s ascend.completed matches 1 if score ascend.RedRaccoons ascend.completed matches 0..3 run \
        function stmc:games/trials/ascend/increment_room
