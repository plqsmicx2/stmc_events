...
execute in stmc:race if score race.handler race.timer.game matches 1 run fill -15 106 10 -11 109 10 air replace minecraft:spruce_fence
...
execute if score race.handler race.timer.game matches 1 run scoreboard players set @a race.return 0
...
execute as @a unless score @s race.laps_completed matches 3.. run function stmc:games/race/checkpoint_update
...
execute as @a if score @s race.return matches 1.. as @s in stmc:race run function stmc:games/race/helper/checkpoint_return
scoreboard players set @a race.return 0

# grant elytra
execute in stmc:race as @a[x=-17,y=125,z=-10,dx=7,dy=3,dz=4] run item replace entity @s armor.chest with elytra[equippable={slot:"chest",asset_id:"armadillo_scute"},enchantments={"minecraft:binding_curse":1}]
execute in stmc:race as @a[x=-17,y=125,z=-10,dx=7,dy=3,dz=4] run item replace entity @s armor.legs with slime_ball[equippable={slot:"legs",camera_overlay:"stmc:overlay/elytra_icon",equip_sound:"item.armor.equip_elytra"},minecraft:enchantments={binding_curse:1,vanishing_curse:1}]
# teleport players back to checkpoint if they're out of bounds
execute in stmc:race as @a at @s if score @s race.checkpoints_completed matches 8 if entity @s[x=-17,dx=-100] run tp @s -14 126 -10
execute in stmc:race as @a at @s if score @s race.checkpoints_completed matches 8 if entity @s[z=57,dz=50] run tp @s -14 126 -10
execute in stmc:race as @a at @s if score @s race.checkpoints_completed matches 8 if entity @s[x=-11,dx=100,z=-7,dz=-50] run tp @s -14 126 -10
execute in stmc:race as @a at @s if score @s race.checkpoints_completed matches 8 if entity @s[x=57,dx=50] run tp @s -14 126 -10

# grant boost
execute in stmc:race as @a[x=-3,y=120,z=8,dx=3,dy=3] run item replace entity @s weapon.mainhand with \
