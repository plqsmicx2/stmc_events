...
execute as @a in trials:spleef run spawnpoint @s ~ ~ ~
...
execute as @a[scores={spleef.alive=1}] if score spleef.handler spleef.stage matches 5 if score spleef.handler spleef.deathThisTick matches 1 run scoreboard players add @s spleef.points.indiv 5
execute as @a[scores={spleef.alive=1}] if score spleef.handler spleef.stage matches 5 if score spleef.handler spleef.deathThisTick matches 1 at @s run playsound minecraft:block.amethyst_cluster.hit master @s ~ ~ ~ 10.0 0.2
execute as @a[scores={spleef.alive=1}] if score spleef.handler spleef.stage matches 5 if score spleef.handler spleef.deathThisTick matches 1 run tellraw @s ["",{text:"[",bold:true,color:"gray"},{text:"+5 points",bold:true,color:"red"},{text:"]",bold:true,color:"gray"},{text:" Outlasted Player!",bold:true,color:"red"}]

# and run points update to give bonus placement points if applicable
execute if score spleef.handler spleef.stage matches 5 if score spleef.handler spleef.deathThisTick matches 1 run function trials:spleef/points_update
