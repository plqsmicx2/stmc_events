...
$execute if score sprint.handler sprint.stats.playersCompleted matches $(players) run function sprint:helper/end_game

execute store result score $max sprint.timer.game run data get storage sprint:data max_time
execute if score sprint.handler sprint.timer.game = $max sprint.timer.game run function sprint:helper/end_game
