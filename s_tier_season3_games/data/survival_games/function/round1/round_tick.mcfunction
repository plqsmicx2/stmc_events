...
function survival_games:round1/border
...
execute in survival_games:sg1 if score sg.r1.handler sg.r1.timer.round matches 1 run fill 8 170 8 -8 170 -8 air replace tinted_glass
...
execute as @a if score sg.r1.handler sg.r1.timer.round matches 60.. if items entity @s armor.chest minecraft:elytra unless entity @s[nbt={FallFlying:1b}] run item replace entity @s armor.chest with minecraft:air

# after 10 seconds remove immunity
execute as @a if score sg.r1.handler sg.r1.timer.round matches 200 run attribute @s attack_damage base set 1.0
execute as @a if score sg.r1.handler sg.r1.timer.round matches 200 run attribute @s knockback_resistance base set 0.0
execute if score sg.r1.handler sg.r1.timer.round matches 200 run effect clear @a resistance
execute if score sg.r1.handler sg.r1.timer.round matches 200 run tellraw @a {"text":"Grace Period has ended.","color":"dark_red"}

# check if game is over & call reset if it is
execute if score sg.r1.handler sg.teams_alive matches 1 run function survival_games:round1/reset