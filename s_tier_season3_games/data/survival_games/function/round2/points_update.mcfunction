...
scoreboard players set sg.YellowYaks sg.r2.points 0
execute as @a[team=YELLOW_YAKS] at @s run scoreboard players operation sg.YellowYaks sg.r2.points += @s sg.r2.points
...
execute as @a if score sg.r2.handler sg.r2.stage matches 1..2 run title @s actionbar [{"text":"Kills: ","color":"yellow"},{"score":{"name":"@s","objective":"sg.r2.kills"},"color":"red"},{"text":"   |   Individual Points: ","color":"yellow"},{"score":{"name":"@s","objective":"sg.points.indiv"},"color":"red"},{"text":"   |   Distance From Border: ","color":"yellow"},{"score":{"name":"@s","objective":"sg.borderDistance"},"color":"red"}]