...
tellraw @a [{selector:"@s"},{text:" set a new Brawl points record!",color:green,bold:true}]

# store @s's points
execute store result storage stmc:records brawl.indiv.value int 1 run scoreboard players get @s brawl.points.indiv
...
data modify storage stmc:records brawl.indiv.name set from entity @n[type=item_display,tag=profile] \
